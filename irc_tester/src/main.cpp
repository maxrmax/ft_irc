...
// runtime flag to control reconnect behavior; default false
bool gReconnectOnMsg = false;
...
int usage( std::string prg ) {
    ...
    std::cout << prg.substr( prg.find_last_of( "/" ) + 1 ) << " <clients> <serverip> <port> [rejoinflag] " << std::endl;
    ...
    std::cout << "  [flag] = no 5th argument, no reconnecting" << std::endl;
...
int main( int ac, char** av ) {
    ...
    ipAddress = av[2];
    port = std::stoi( av[3] );
    // optional permissive flag: if provided, enable reconnect
    if ( ac == 5 )
        gReconnectOnMsg = true;
